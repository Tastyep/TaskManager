...
ThreadManager::ThreadManager(unsigned int nbThread)
  : running(true) {
...
unsigned int ThreadManager::roundToNextPower(unsigned int nbThread) const {
  nbThread |= nbThread >> 1;
  nbThread |= nbThread >> 2;
  nbThread |= nbThread >> 4;
  nbThread |= nbThread >> 8;
...
std::shared_ptr<Worker> ThreadManager::getWorker() {
...
std::pair<bool, std::string> ThreadManager::stop() {
  std::lock_guard<std::mutex> guard(this->workersMutex);
