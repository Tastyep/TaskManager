...
std::pair<bool, std::string> ThreadPool::start() {
  if (this->status.load(std::memory_order_seq_cst) != state::STOP) {
...
std::pair<bool, std::string> ThreadPool::pause() {
...
std::pair<bool, std::string> ThreadPool::unpause() {
...
std::pair<bool, std::string> ThreadPool::stop() {
  bool waitCondition;
  if (this->status.load(std::memory_order_seq_cst) == state::STOP) {
...
void ThreadPool::resize(unsigned int maxParallelism) {
...
void ThreadPool::startTask() {
  ...
  if (this->taskContainer.empty() || this->threadRefCount >= this->maxParallelism ||
