...
void Scheduler::removeWorkerRef(std::shared_ptr<Worker> worker) {
  std::lock_guard<std::mutex> guardWorker(this->workerMutex);
  this->workers.erase(
    std::remove_if(this->workers.begin(), this->workers.end(), [worker](const auto& w) { return w == worker; }),
    this->workers.end());
}

void Scheduler::decreaseRefCount() {
  std::lock_guard<std::mutex> guardRef(this->refCountMutex);
  --(this->threadRefCount);
}

std::pair<Task, std::chrono::steady_clock::time_point> Scheduler::getHighestPriorityTask() {
...
void Scheduler::runAt(const Task& task, const std::chrono::steady_clock::time_point& timePoint) {
  this->addTask(task, timePoint);
}

void Scheduler::runIn(const Task& task, const std::chrono::steady_clock::duration& duration) {
  this->addTask(task, std::chrono::steady_clock::now() + duration);
}

void Scheduler::runEvery(const Task& task, const std::chrono::steady_clock::duration& duration) {
...
void Scheduler::addTask(const Task& task, const std::chrono::steady_clock::time_point& timePoint) {
...
std::pair<bool, std::string> Scheduler::pause() {
...
std::pair<bool, std::string> Scheduler::unpause() {
